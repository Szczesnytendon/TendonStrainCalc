...
function[DICmesh,ResultDisp,RTx,RTy] = main_ALDIC(iteration,path_img)
    % this function performs DIC on two images.
    % The output DICmesh holds the nodal positions, and ResultDisp is the
    % displacements at such points. The inputs are iteration, which defines
    % the pair of images to use, and path_img indicates where the images
    % should come from.
    ...
    % ====== Read images ======
    imgfoldername = path_img;
    [file_name,Img,DICpara] = ReadImage(imgfoldername,iteration); % Load DIC raw images
    % %%%%%% Uncomment the line below to change the DIC computing region (ROI) manually %%%%%%
    %DICpara.gridxROIRange = [gridxROIRange(1),gridxROIRange(2)]; DICpara.gridyROIRange = [gridyROIRange(1), gridyROIRange(2)];
    ...
    for ImgSeqNum = 2 : length(ImgNormalized)
        ...
        moving = ImgNormalized{ImgSeqNum}; % Load current deformed image frame 
        tform = imregcorr(moving,fNormalized,'translation');
        gNormalized = imtranslate(moving, [tform.T(3,1), tform.T(3,2)]);
        RTx = tform.T(3,1);
        RTy = tform.T(3,2);
        ...
        if ImgSeqNum == 2 || DICpara.NewFFTSearch == 1 % Apply FFT-based cross correlation to compute the initial guess 
            % ====== Integer Search ======
            [DICpara,x0temp,y0temp,u,v,cc]= IntegerSearch(fNormalized,gNormalized,file_name,DICpara,iteration);
            % ====== FEM mesh set up ======
