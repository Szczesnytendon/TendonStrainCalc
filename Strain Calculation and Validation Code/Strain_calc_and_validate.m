...
%% Input Parameters
prompt = {'Enter (Even) Subset Size (For Nuclear Tracking):',...
    'Enter Subset Step Size (0.25*Subset Size-1*Subset Size):',... 
    'Method to solve ALDIC global step Subproblem 2:                       1: Finite Difference (Recommended)                                         2: Finite Element',...
    'How many parallel pools to open (1 if no parallel computing)',...
    'Method to compute an inital guess of displacements                    0: Multigrid search based on image pyramid                                 1: Whole field search for all subsets                                      2: Search near manually clicked seeds and then interpolate for full field'};
dlgtitle = 'ALDIC Parameters';
dims = [1 70];
definput = {'20','10','1','1','0'};
global answerDIC
answerDIC = str2double(inputdlg(prompt,dlgtitle,dims,definput));


prompt = {'Enter Correlation Coeffecient Threshold to Identify "Bad" Regions:',...
    'Enter Subsize for Search Window:',... 
    'Enter Number of Neighboring Points Included in Strain Calculations:'};
dlgtitle = 'Bad Region and Strain Analysis Parameters';
dims = [1 90];
definput = {'0.5','32','12'};
answerBRA = str2double(inputdlg(prompt,dlgtitle,dims,definput));

fig=uifigure('Position',[500 300 300 300]);
lbl=uilabel(fig,'Text','Should means, stdevs, and plots be automatically saved?','Position',[20 140 300 250],'WordWrap','on');
lbl2=uilabel(fig,'Text','Which variables?','Position',[20 80 300 250],'WordWrap','on');
cbxY=uicheckbox(fig,'Text','Yes','Position',[20 220 100 20],'Value',1);
cbx1=uicheckbox(fig,'Text','X-Strain','Position',[20 170 100 20],'Value',1);
cbx2=uicheckbox(fig,'Text','Y-Strain','Position',[20 150 100 20],'Value',1);
cbx3=uicheckbox(fig,'Text','Shear Strain','Position',[20 130 100 20],'Value',1);
cbx4=uicheckbox(fig,'Text','Max Principal Strain','Position',[20 110 200 20],'Value',1);
cbx5=uicheckbox(fig,'Text','Min Principal Strain','Position',[20 90 200 20],'Value',1);
cbx6=uicheckbox(fig,'Text','Max Shear Strain','Position',[20 70 150 20],'Value',1);
cbx7=uicheckbox(fig,'Text','Bad Regions','Position',[20 50 100 20],'Value',1);
myButtonTest(fig,cbx1,cbx2,cbx3,cbx4,cbx5,cbx6,cbx7,cbxY);
global results;

% Define correlation coefficient threshold to identify "bad" regions:
corr_threshold = answerBRA(1);
...
for l = 1:numImg-1
    ...
    for k=1:size(Bxy,1)
        ...
        for ii=1:length(inds)
            ...
            maxS(k) = abs(((maxP(k)-1)-(minP(k)-1))./2);
    end
    ...
    % Plotting maximum shear strains
    f8 = figure;
    set(gcf,'Name','Max Shear Strain')
    temp=reshape(maxS,vsize(2),vsize(1))';
    contourf(Xn,Yn,temp)
    colorbar
    set(gca, 'ydir','reverse')
    ...
    if results(8) == 1
        if l == 1
            countS = 1+sum([2 2 2 2 2 2 1 0].*results);
            Storage=cell(countS,numImg);
            for zzz = 1:(numImg-1)
        ...
        switch results(1)
            case 1
                ...
                Storage{2,l+1} = mean(epsilonxx,'all','omitnan');
        ...
        switch results(2)
            case 1
                Storage{progress+1,l+1} = mean(epsilonyy,'all','omitnan');
                Storage{progress+2,l+1} = std(epsilonyy,'omitnan');
                f4name = sprintf('Y Strain 0-%d',count);
                saveas(f4,f4name)
                if l==1
                    Storage{progress+1,1} = 'Av Y Strain';
                    Storage{progress+2,1} = 'Y Std';
                end
                progress=progress+2;
            case 0
                progress=progress+0;
        ...
        switch results(3)
            case 1
                Storage{progress+1,l+1} = mean(gamma,'all','omitnan');
                Storage{progress+2,l+1} = std(gamma,'omitnan');
                f5name = sprintf('Shear Strain 0-%d',count);
                saveas(f5,f5name)
                if l==1
                    Storage{progress+1,1} = 'Av Shear Strain';
                    Storage{progress+2,1} = 'Shear Std';
                end
                progress=progress+2;
            case 0
                progress=progress+0;
        ...
        switch results(4)
            case 1
                Storage{progress+1,l+1} = mean(maxP-1,'all','omitnan');
                Storage{progress+2,l+1} = std(maxP-1,'omitnan');
                f6name = sprintf('Max Principal Strain 0-%d',count);
                saveas(f6,f6name)
                if l==1
                    Storage{progress+1,1} = 'Av Max Prin Strain';
                    Storage{progress+2,1} = 'Max Prin Strain Std';
                end
                progress=progress+2;
            case 0
                progress=progress+0;
        ...
        switch results(5)
            case 1
                Storage{progress+1,l+1} = mean(minP-1,'all','omitnan');
                Storage{progress+2,l+1} = std(minP-1,'omitnan');
                f7name = sprintf('Min Principal Strain 0-%d',count);
                saveas(f7,f7name)
                if l==1
                    Storage{progress+1,1} = 'Av Min Prin Strain';
                    Storage{progress+2,1} = 'Min Prin Strain Std';
                end
                progress=progress+2;
            case 0
                progress=progress+0;
        ...
        switch results(6)
            case 1
                Storage{progress+1,l+1} = mean(maxS,'all','omitnan');
                Storage{progress+2,l+1} = std(maxS,'omitnan');
                f8name = sprintf('Max Shear Strain 0-%d',count);
                saveas(f8,f8name)
                if l==1
                    Storage{progress+1,1} = 'Av Max Shear Strain';
                    Storage{progress+2,1} = 'Max Shear Strain Std';
                end
                progress=progress+2;
            case 0
                progress=progress+0;
        ...
        switch results(7)
            case 1
                Storage{progress+1,l+1} = 100*(total-good)/total;
                f1name = sprintf('Bad Regions 0-%d',count);
                saveas(f1,f1name)
                if l==1
                    Storage{progress+1,1} = 'Bad Region %';
                end
                progress=progress+1;
            case 0
                progress=progress+0;
